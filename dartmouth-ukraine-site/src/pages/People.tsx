...
export default function People() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [people, alums] = await Promise.all([
    ...
    <div className="mx-auto max-w-5xl px-4 py-10 space-y-16">
      <section>
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((p) => <PersonCard key={p._id} p={p} />)}
        </div>
      </section>

      <section>
        <h2 className="text-2xl font-semibold tracking-tight">Alumni</h2>
        <p className="mt-2 text-gray-600">Dartmouth graduates who have been a part of the club.</p>
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {alumni.map((p) => <PersonCard key={p._id} p={p} />)}
        </div>
      </section>
    </div>
  );
