...
export default function Home() {
  ...
  return (
    <div className="space-y-16 md:space-y-24">
      {/* HERO */}
      <section className="text-center space-y-6 pt-8 md:pt-14">
        <img src={flagImg} alt="Flag of Ukraine" className="mx-auto w-20 md:w-24 rounded-sm shadow-sm" />
          ...
          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm text-left overflow-hidden">
            <div className="p-4 md:p-6">
              <div className="flex items-center justify-between gap-4">
                ...
                <a
                  href="/news"
                  className="text-sm font-medium text-slate-700 hover:text-slate-900 underline underline-offset-4 min-h-[44px] flex items-center"
                >
                  View all
                </a>
              </div>
              ...
              {loading ? (
                ...
                <div className="mt-4 text-slate-600">No articles yet — check back soon.</div>
              ) : (
                <a
                  href={`/news/${latest.slug}`}
                  className="mt-4 block rounded-xl hover:bg-slate-50 transition p-2 -mx-2"
                >
                  <div className="flex gap-4 items-start">
                    {latest.coverImage ? (
                      <img
                        ...
                        className="w-24 h-16 md:w-40 md:h-28 rounded-xl object-cover border border-slate-200 flex-shrink-0"
                        loading="lazy"
                      />
                    ) : (
                      <div className="w-24 h-16 md:w-40 md:h-28 rounded-xl bg-slate-100 border border-slate-200 flex-shrink-0" />
                    )}

                    <div className="min-w-0">
                      <div className="text-base md:text-xl font-semibold text-slate-900 leading-snug line-clamp-2">
                      ...
                      {latest.excerpt ? (
                        ...
                        <p className="mt-2 text-sm text-slate-600">Read the latest update from DSAU.</p>
                      )}
      ...
      <section className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
        <div className="space-y-4 text-center md:text-left">
          ...
          <p className="text-slate-600 leading-relaxed">
            ...
            and create meaningful dialogue on campus about Ukrainian culture and sovereignty.
          </p>
          <p className="text-slate-600 leading-relaxed">
      ...
      <section className="text-white rounded-xl p-8 md:p-12 text-center space-y-6" style={{backgroundColor: '#1e3a8a'}}>
        ...
        <a
          href="/contact"
          className="inline-flex items-center justify-center px-6 py-3 min-h-[44px] bg-white text-slate-900 rounded-md font-medium hover:bg-slate-100 transition"
        >
          Join the Club
        </a>
      </section>
